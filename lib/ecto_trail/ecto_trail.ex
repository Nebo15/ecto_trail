defmodule EctoTrail do
  ...
  @doc """
  Call `c:Ecto.Repo.delete/2` operation and store changes in a `change_log` table.

  Insert arguments, return and options same as `c:Ecto.Repo.delete/2` has.
  """
  @spec delete_and_log(
          repo :: Ecto.Repo.t(),
          struct_or_changeset :: Ecto.Schema.t() | Ecto.Changeset.t(),
  ...
  defp run_logging_transaction(multi, repo, struct_or_changeset, actor_id, type_operation) do
    multi
  ...
  defp log_changes(repo, multi_acc, struct_or_changeset, actor_id, operation_type) do
    ...
    # |> case do
    #   %{} ->
    #     IO.puts "llego a delete"
    #     {_, regreso } = Map.from_struct(struct_or_changeset)|>
    #     Map.pop(:__meta__)
    #     IO.inspect regreso
    #     regreso
    #   change_struct ->
  ...
  defp validate_changes(changes, schema, operation_type) do
    case operation_type do
      ...
      2->
        changes
      3 ->
        {_, regreso } = Map.from_struct(schema)|>
        Map.pop(:__meta__)
        IO.inspect regreso
  ...
  defp remove_empty_assosiations(struct)do
    new_struct =
    Enum.map(struct, fn ({key, value}) -> {key,
